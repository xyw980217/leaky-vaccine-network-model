...
Disease-like robustness of the high-exposure strategy.

The figure omits panel letters and outer borders while retaining cell annotations, axes, and colour scales.
"""
...
OUTPUT_PDF = OUTPUT_DIR / "FigureS4.pdf"
OUTPUT_PNG = OUTPUT_DIR / "FigureS4.png"
...
def _is_high_exposure_strategy(value: object) -> bool:
    key = _normalise_name(value)
    return (
        key in {
            "high_strength_(node)",
            "high_strength_node",
            "high_exposure",
            "high_exposure_strategy",
            "priority:_high_strength_(node)",
        }
        or (
            "high" in key
            and ("strength" in key or "exposure" in key)
            and "neighbor" not in key
        )
    )
...
def prepare_summary_table(
    ...
    out = out[
        ...
        & out["Strategy"].map(_is_high_exposure_strategy)
    ...
    print(f"[INFO] High-exposure plotted values: {len(out)}")
...
def plot_figure(
    ...
    for row_idx, outcome in enumerate(OUTCOME_ORDER):
        ...
        for col_idx, scenario in enumerate(SCENARIO_ORDER):
            ...
            # Retain white internal cell borders.
            ax.set_xticks(
        ...
        # Remove the colour-bar outline and any axes spines.
