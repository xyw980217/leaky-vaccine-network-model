"""Quota-based all-or-nothing allocation experiments for Figure S4."""
...
SHEET_ID = 0
OUTPUT_ROOT = os.environ.get('LEAKY_OUTPUT', str(Path(__file__).resolve().parents[1] / 'results' / 'hybrid_all_or_nothing'))
...
if __name__ == '__main__' and os.path.exists(OUTPUT_ROOT):
    if not os.path.isdir(OUTPUT_ROOT) or os.listdir(OUTPUT_ROOT):
        raise FileExistsError('Output path is not an empty directory: ' + OUTPUT_ROOT + '\nChoose a NEW OUTPUT_ROOT; existing results are never replaced.')
...
COVERAGE = 0.5
R0_LIST = [float(x) for x in os.environ['LEAKY_R0'].split(',')] if 'LEAKY_R0' in os.environ else [1.0, 7.0, 18.6]
...
FIG5_CD_R0_LIST = [r for r in [1.0, 7.0, 18.6] if r in R0_LIST]
...
def run_single_hybrid_point(task_index, current_ves0, target_r0, alpha):
    ...
    aon_take_uniform_matrix = np.zeros((MC_RUNS_PER_POINT, N), dtype=np.float64)
    aon_take_seeds = np.zeros(MC_RUNS_PER_POINT, dtype=np.uint32)
    aon_stage_protected_counts = np.zeros((MC_RUNS_PER_POINT, 3), dtype=np.int32)
    aon_stage_seed_overlaps = np.zeros((MC_RUNS_PER_POINT, 3), dtype=np.int32)
    aon_stage_efficacies = np.array([current_ves0, 0.7 * current_ves0, 0.4 * current_ves0], dtype=float)
    for mc_run_id in range(1, MC_RUNS_PER_POINT + 1):
        _, aon_take_uniforms, aon_take_seed = get_aon_protected_mask(vax_mask, current_ves0, target_r0, COVERAGE, mc_run_id)
        unique_infected_total, peak_size, peak_time, expected_deaths, daily_curve, ever_infected_mask, cumulative_exposure_arr, seed_node_indices, final_ever_by_strength_decile, peak_infectious_by_strength_decile = run_strategy_simulation(current_beta, vax_mask, current_ves0, aon_take_uniforms)
        row_id = mc_run_id - 1
        aon_take_uniform_matrix[row_id] = aon_take_uniforms
        aon_take_seeds[row_id] = aon_take_seed
        for stage_id, stage_ve in enumerate(aon_stage_efficacies):
            protected = vax_mask & (aon_take_uniforms < stage_ve)
            aon_stage_protected_counts[row_id, stage_id] = int(protected.sum())
            aon_stage_seed_overlaps[row_id, stage_id] = int(protected[seed_node_indices].sum())
    ...
    replicate_df = pd.DataFrame({'VES_0': current_ves0, 'R0': target_r0, 'Coverage': COVERAGE, 'Actual Coverage': actual_coverage, 'Alpha High Exposure': alpha, 'Alpha Oldest First': 1.0 - alpha, 'High Exposure Discordant-Quota Fraction': alpha, 'Oldest First Discordant-Quota Fraction': 1.0 - alpha, 'MC_Run': np.arange(1, MC_RUNS_PER_POINT + 1), 'Simulation Seed': seed, 'Unique Infected Total': unique_infected_totals, 'Final Infection Proportion': infection_rates, 'Peak Size': peak_sizes, 'Peak Time': peak_times, 'Expected Deaths': expected_deaths_values, 'Vaccine Failure Mode': VACCINE_FAILURE_MODE, 'AON Take Seed': aon_take_seeds, 'AON Protected Count Days 1-100': aon_stage_protected_counts[:, 0], 'AON Protected Count Days 101-180': aon_stage_protected_counts[:, 1], 'AON Protected Count Days 181-500': aon_stage_protected_counts[:, 2], 'Initial Seed / AON Protected Overlap Days 1-100': aon_stage_seed_overlaps[:, 0], 'Initial Seed / AON Protected Overlap Days 101-180': aon_stage_seed_overlaps[:, 1], 'Initial Seed / AON Protected Overlap Days 181-500': aon_stage_seed_overlaps[:, 2]})
...
def build_fig5_cd_endpoint_decomposition():
    ...
    net_rows = []
    validation_rows = []
    paired_replicate_frames = []
    required_arrays = {'Seed_Node_Indices', 'AON_Take_Uniforms', 'AON_Take_Seeds', 'Final_Ever_Infected_By_Strength_Decile', 'Peak_Infectious_By_Strength_Decile', 'Final_Infection_Proportion', 'Peak_Size', 'Total_Strength_Decile_Node_Counts'}
    for r0 in FIG5_CD_R0_LIST:
        ...
        with np.load(oldest_path, allow_pickle=False) as oldest, np.load(high_path, allow_pickle=False) as high:
            ...
            take_seeds_equal = np.array_equal(oldest['AON_Take_Seeds'], high['AON_Take_Seeds'])
            take_uniforms_equal = np.array_equal(oldest['AON_Take_Uniforms'], high['AON_Take_Uniforms'])
            if not (take_seeds_equal and take_uniforms_equal):
                raise RuntimeError(f'R0={r0:g}: endpoint AON take draws are not paired')
            ...
            paired_replicates = pd.DataFrame({'R0': r0, 'VES_0': VES_0_LIST[0], 'Coverage': COVERAGE, 'MC_Run': np.arange(1, len(net_fip) + 1), 'FIP_High_Minus_Oldest_pp': net_fip, 'Peak_High_Minus_Oldest_People': net_peak, 'Expected_Deaths_High_Minus_Oldest': high['Expected_Deaths'] - oldest['Expected_Deaths']})
            for group_id in range(10):
                paired_replicates[f'FIP_Decile_{group_id + 1}_Contribution_pp'] = fip_contribution[:, group_id]
                paired_replicates[f'Peak_Decile_{group_id + 1}_Contribution_People'] = peak_contribution[:, group_id]
            paired_replicate_frames.append(paired_replicates)
            for group_id in range(10):
            ...
            validation_rows.append({'R0': r0, 'Paired_Seed_Sets_Equal': seeds_equal, 'Paired_AON_Take_Seeds_Equal': take_seeds_equal, 'Paired_AON_Take_Uniforms_Equal': take_uniforms_equal, 'FIP_Contribution_Max_Abs_Error_pp': fip_error, 'Peak_Contribution_Max_Abs_Error_People': peak_error, 'Replicates': int(len(net_fip)), 'Deciles': 10, 'Nodes': N})
    ...
    paired_replicate_path = os.path.join(FIG5_CD_DIR, 'Fig5_CD_All_Paired_Replicate_Differences.csv.gz')
    pd.concat(paired_replicate_frames, ignore_index=True).to_csv(paired_replicate_path, index=False, compression='gzip')
    ...
    return {'paired_replicates': paired_replicate_path, 'decomposition': decomposition_path, 'net': net_path, 'validation': validation_path, 'panel_c': plot_ready_paths['FIP_Contribution_pp'], 'panel_d': plot_ready_paths['Peak_Contribution_People']}
if __name__ == '__main__':
    print(f'\n{'=' * 72}', flush=True)
    print('AON QUOTA-MIXTURE PEAK-vs-FINAL-INFECTION ANALYSIS', flush=True)
