"""Run vaccination-strategy robustness experiments across network structures.

The script evaluates the survey-derived reference network, three structural
perturbations, and three idealized topology controls.
"""
import os
from pathlib import Path

# Limit implicit low-level threading to avoid nested parallelism with ProcessPoolExecutor.
...
OUTPUT_DIR = PROJECT_ROOT / 'results' / 'network_structure_robustness'
...
# --- COVID-like baseline only for network-structure robustness ---
...
# Keep this grid compact: the goal is network robustness, not a full parameter sweep.
...
# Audited stochastic-comparison settings.
...
# Network replicate settings.
# Main-text networks: first four network types below.
NETWORK_REPLICATES_MAIN = 10
NETWORK_REPLICATES_SI = 5

# Set to False if you only want tables and summary figures.
SAVE_DYNAMICS_CURVES = False
SAVE_REPLICATE_OUTCOMES = True
SAVE_AGE_GROUP_COUNTS = True

# --- Network and exposure parameters ---
HH_CONTACT_HOURS = 12.0
SOCIAL_ENCOUNTER_DURATION = 0.25
INTERNAL_SOCIAL_RATIO = 0.3  # Fraction of non-fixed activity assigned to random contacts within the observed study sample
VIRTUAL_CONTACT_MAPPING = {'a': 3.0, 'b': 10.0, 'c': 20.0, 'd': 30.0}
EXTERNAL_PREVALENCE = 0.005
EXT_SOCIAL_CONTACT_DENSITY = 4.0

# --- Outcome and strategy settings ---
strategies_list = [
    'Random',
    'High Strength (Node)',
...
# Network types. The first four are intended for the main text; the last three for SI.
MAIN_TEXT_NETWORK_TYPES = [
    'Empirical',
    'CommunityResampled',
    'SmallWorldCommunity',
...
# IFR mapping used for expected deaths, kept the same as the main COVID-like baseline.
IFR_MAP = {
    15:   0.0000695,
    24:   0.000309,
    34.5: 0.000844,
...
def make_stable_seed(*items, base_seed=RNG_SEED):
    """Create a deterministic seed stable across Python sessions."""
...
def descending_order_with_explicit_ties(values):
    """
    Return indices ordered by descending value, with descending node ID as an
    explicit deterministic tie-breaker. This removes dependence on NumPy's
    unspecified quicksort ordering for equal values.
    """
...
def summarize_metric(values, prefix):
    ...
    return {
        prefix: mean_val,
        '{} SD'.format(prefix): sd_val,
        '{} SE'.format(prefix): se_val,
        '{} CI95 Lower'.format(prefix): mean_val - 1.96 * se_val if n_obs > 0 else np.nan,
        '{} CI95 Upper'.format(prefix): mean_val + 1.96 * se_val if n_obs > 0 else np.nan,
    }
...
def gini_coefficient(x):
    """Memory-efficient O(n log n) Gini coefficient for non-negative values."""
...
def calibrate_beta(target_r0, gamma, graph, n_nodes):
    """Calibrate beta from the weighted spectral radius; never silently fall back."""
    sr = spectral_radius_weighted(graph, n_nodes)
    if not np.isfinite(sr) or sr <= 0:
        raise RuntimeError(
            'Invalid weighted spectral radius {}; calibration aborted.'.format(sr)
        )
    return (target_r0 * gamma * 24.0) / sr

# ============================================
# 3. Load data and construct node attributes
# ============================================
...
print(
    '[HOUSEHOLD CHECK] Unique households: {}; households with >=2 respondents: {}; '
    'duplicate household-member rows: {}'.format(
        df['_household_key'].nunique(),
        int((household_sizes >= 2).sum()),
        int(duplicated_member_mask.sum()),
    ),
    flush=True,
)
if duplicated_member_mask.any() and not ALLOW_DUPLICATE_HOUSEHOLD_MEMBER_KEYS:
    duplicate_keys = (
        df.loc[duplicated_member_mask, '_household_member_key']
        .value_counts()
        .head(20)
        .to_dict()
    )
    raise ValueError(
        'Duplicated community + address + household-member-index keys were '
        'detected. Resolve the source data or explicitly set '
        'ALLOW_DUPLICATE_HOUSEHOLD_MEMBER_KEYS=True after justification. '
        'Examples: {}'.format(duplicate_keys)
    )
...
# Pre-compute individual community encounter budgets.
internal_social_time = daily_hours_nonfixed * INTERNAL_SOCIAL_RATIO
community_encounter_counts = np.array([
    int(round(t / SOCIAL_ENCOUNTER_DURATION)) if t > 0 else 0
    for t in internal_social_time
], dtype=int)
...
def add_random_budget_community_edges(graph, rng, weight_scale=1.0):
    # Candidate partners are all other residents in the observed study sample,
    # matching the main experiment code; targets are not restricted to the same
    # residential-community label.
    ...
    for i in range(N):
        ...
        for t in tgts:
            if graph.has_edge(i, int(t)):
                ...
                # Preserve home label if already home; otherwise mark community.
...
REFERENCE_NUM_NONHOME_EDGES = sum(
    1 for _, _, data in REFERENCE_EMPIRICAL_GRAPH.edges(data=True)
    if data.get('layer') != 'home'
)
...
print('[NETWORK] Reference empirical network built. Edges={}, Community edges={}, Avg internal strength={:.2f}'.format(
    REFERENCE_NUM_EDGES, REFERENCE_NUM_NONHOME_EDGES, REFERENCE_INTERNAL_STRENGTH.mean()), flush=True)
...
def random_unique_edges(rng, num_edges, exclude_existing=None):
    ...
    if len(edges) != num_edges:
        raise RuntimeError(
            'Could not generate the requested number of unique random edges: '
            'requested {}, generated {}.'.format(num_edges, len(edges))
        )
    return list(edges)
...
def build_small_world_community_graph(rng):
    ...
    ws = nx.watts_strogatz_graph(N, k, 0.10, seed=ws_seed)

    # Randomly relabel the ring nodes so that spreadsheet row order cannot
    # determine small-world neighbourhoods.
...
def build_ba_scale_free_graph(rng):
    ...
    # In BA, number of edges ~= m * (N - m). Choose m to approximate target edge count.
    m = max(1, int(round(target_edges / float(N))))
    m = min(m, max(1, N - 1))
    ba_seed = int(rng.randint(0, 2**31 - 1))
    ba = nx.barabasi_albert_graph(N, m, seed=ba_seed)

    # Randomly relabel nodes because the initial BA seed clique otherwise
    # privileges low spreadsheet row IDs.
    permutation = rng.permutation(N)
    mapping = {old: int(permutation[old]) for old in range(N)}
...
def validate_network_graph(graph, network_type, replicate_id):
    """Fail fast on malformed robustness networks."""
    if graph.number_of_nodes() != N:
        raise RuntimeError(
            'Network {} replicate {} has {} nodes; expected {}.'.format(
                network_type, replicate_id, graph.number_of_nodes(), N
            )
        )
    if nx.number_of_selfloops(graph) != 0:
        raise RuntimeError(
            'Network {} replicate {} contains self-loops.'.format(
                network_type, replicate_id
            )
        )
    weights = np.array(
        [float(data.get('weight', np.nan)) for _, _, data in graph.edges(data=True)],
        dtype=float,
    )
    if weights.size == 0 or not np.all(np.isfinite(weights)) or np.any(weights <= 0):
        raise RuntimeError(
            'Network {} replicate {} contains missing, non-finite or non-positive weights.'.format(
                network_type, replicate_id
            )
        )
...
def make_network_context(network_type, replicate_id, graph):
    ...
    return {
        ...
        'Network_Replicate': replicate_id,
        'G': graph,
        'adj_matrix': adj_matrix,
        'strength_internal': strength_internal,
        'total_node_strength': total_node_strength,
...
def compute_network_diagnostics(context):
    ...
    return {
        ...
        'Network_Replicate': context['Network_Replicate'],
        'Number_of_edges': n_edges,
        'Home_edge_fraction': home_edges / n_edges if n_edges > 0 else np.nan,
        'Community_edge_fraction': community_edges / n_edges if n_edges > 0 else np.nan,
        'Home_weight_fraction': home_weight / total_weight if total_weight > 0 else np.nan,
        'Community_weight_fraction': community_weight / total_weight if total_weight > 0 else np.nan,
        'Mean_degree': float(np.mean(degree_arr)),
        'Mean_internal_strength': float(np.mean(strength_internal)),
        'Mean_total_strength': float(np.mean(total_node_strength)),
        'Gini_total_strength': gini_coefficient(total_node_strength),
        'Weighted_spectral_radius': sr,
        'Mean_weighted_clustering': mean_clustering,
        'Degree_assortativity': assortativity,
        'Number_connected_components': n_components,
        'Giant_component_fraction': giant_component_fraction,
        'Average_shortest_path_largest_component': avg_shortest_path_gc,
        'Age_strength_correlation': safe_corr(df['age_num'].values, total_node_strength),
    }
...
def run_strategy_simulation(beta_val, vax_mask, ves_0, context, simulation_seed):
    ...
    for d in range(1, SIM_DAYS_STRATEGY + 1):
        ...
        new_state[exp_indices] = np.where(
            r_val[exp_indices] < SCENARIO_PARAMS['PROB_E_TO_I'],
            2,
            1,
        )
        ...
        # Recovery occurs at the end of the current daily step. Incrementing
        # immediately means that the next simulated day begins at day 1 after
        # recovery, matching the main experiment's daily-step convention.
...
# ============================================
# 6. Worker and summary functions
# ============================================
    ...
    for ves0 in VES_0_LIST:
        for target_r0 in R0_LIST:
            ...
            for cov_idx, cov in enumerate(COVERAGE_STEPS, start=1):
                print(
                    '[PROGRESS] Network={}, rep={}, VES_0={}, R0={}, Coverage {}/{}={:.2f}, Elapsed={:.1f} min'.format(
                        network_type, replicate_id, ves0, target_r0, cov_idx, len(COVERAGE_STEPS), cov,
                ...
                is_target_point = np.isclose(cov, 0.50, atol=0.01)

                for strat in strategies_to_run:
                    # Vaccinated sets use a setting-specific RNG independent of
                    # epidemic random streams and remain fixed across MC runs.
                    mask_seed = make_stable_seed(
                        'vaccination_mask', network_type, replicate_id,
                        ves0, target_r0, float(cov), strat
                    )
                    ...
                    for mc_run in range(1, MC_RUNS_PER_POINT + 1):
                        # VE_S,0 is inactive at zero coverage, so baseline
                        # seeds deliberately omit VE_S,0. At target coverage,
                        # strategies share the same seed when common random
                        # numbers are enabled.
                        if np.isclose(cov, 0.0, atol=0.01):
                            simulation_seed = make_stable_seed(
                                'epidemic', network_type, replicate_id,
                                target_r0, 0.0, mc_run
                            )
                        else:
                            seed_items = [
                                'epidemic', network_type, replicate_id,
                                ves0, target_r0, float(cov), mc_run
                            ]
                            if not USE_COMMON_RANDOM_NUMBERS:
                                seed_items.append(strat)
                            simulation_seed = make_stable_seed(*seed_items)

                        c, p_sz, p_tm, d, daily_curve, ever_infected_mask = run_strategy_simulation(
                            current_beta,
                            vax_mask,
                            ves0,
                            context,
                        ...
                        if SAVE_REPLICATE_OUTCOMES:
                            replicate_row = {
                                ...
                                'Strategy': strat,
                                'MC_Run': mc_run,
                                'Simulation Seed': int(simulation_seed),
                                'Vaccination Mask Seed': int(mask_seed),
                                'Common Random Numbers': bool(USE_COMMON_RANDOM_NUMBERS),
                    ...
                    temp_results.append({
                        ...
                        **summarize_metric(exp_deaths, 'Expected Deaths'),
                        'Adjusted_R0_Internal': float(adjusted_target_r0),
            ...
            for outcome in OUTCOME_COLUMNS:
                ...
                rows.append({
                    **key_dict,
                    ...
                    'Positive Point Estimate': float(delta > 0) if not np.isnan(delta) else np.nan,
                })
...
def aggregate_across_network_replicates(delta_df):
    """
    Average additional reduction across stochastic network replicates first.
    The resulting rows correspond to genuine R0 x VE_S,0 parameter combinations,
    rather than treating each network realization as a separate parameter point.
    """
    if delta_df.empty:
        return pd.DataFrame()
    return (
        ...
        .agg(
            Mean_additional_reduction_across_network_replicates=(
                'Additional Reduction (%)', 'mean'
            ),
            SD_across_network_replicates=('Additional Reduction (%)', 'std'),
            N_network_replicates=('Network_Replicate', 'nunique'),
        )
    )
...
def summarize_qualitative_patterns(parameter_level_delta_df):
    """
    Summarize positive point estimates across the 12 R0 x VE_S,0 combinations.
    This is descriptive and does not claim statistical significance.
    """
    ...
    summary = (
        work
        .groupby(['Network_Type', 'Strategy', 'Outcome'], as_index=False)
        .agg(
            N_parameter_combinations=(value_col, 'count'),
            Mean_additional_reduction=(value_col, 'mean'),
            Median_additional_reduction=(value_col, 'median'),
            Fraction_positive_point_estimate=('Positive Point Estimate', 'mean'),
        )
    )
    return summary
...
def plot_network_summary(summary_df, network_types, out_png):
    if summary_df.empty:
        return pd.DataFrame()
    plot_df = summary_df[summary_df['Network_Type'].isin(network_types)].copy()
    if plot_df.empty:
        return pd.DataFrame()
    plot_df['Positive point estimates across parameter combinations (%)'] = 100.0 * plot_df['Fraction_positive_point_estimate']
    plot_df['Network_Type'] = pd.Categorical(plot_df['Network_Type'], categories=network_types, ordered=True)
    plot_df = plot_df.sort_values(['Network_Type', 'Outcome', 'Strategy'])

    g = sns.catplot(
        data=plot_df,
        x='Outcome',
        y='Positive point estimates across parameter combinations (%)',
        hue='Strategy',
        col='Network_Type',
        col_wrap=4 if len(network_types) > 4 else len(network_types),
        kind='bar',
        height=4,
        aspect=1.05,
        palette='deep',
    )
    g.set_axis_labels('Outcome', 'Positive point estimates across parameter combinations (%)')
...
if __name__ == '__main__':
    ...
    with ProcessPoolExecutor(max_workers=MAX_CORES) as executor:
        futures = {
            executor.submit(run_network_replicate, nt, rep): (nt, rep)
            for nt, rep in tasks
        }
    ...
    if failed_tasks:
        raise RuntimeError(
            'One or more network-robustness tasks failed; partial output is not '
            'accepted. Failed tasks: {}'.format(failed_tasks)
        )
    ...
    expected_summary_rows = (
        ...
        * len(VES_0_LIST)
        * len(R0_LIST)
        * (1 + len(strategies_list))
    )
    if STRICT_OUTPUT_VALIDATION:
        if len(final_results_df) != expected_summary_rows:
            raise RuntimeError(
                'Incomplete network summary output: expected {} rows, found {}.'.format(
                    expected_summary_rows, len(final_results_df)
                )
            )
        summary_keys = [
            'Network_Type', 'Network_Replicate', 'VES_0', 'R0',
            'Coverage', 'Strategy'
        ]
        ...
        if len(diagnostics_df) != expected_network_tasks:
            raise RuntimeError(
                'Expected {} network diagnostic rows, found {}.'.format(
                    expected_network_tasks, len(diagnostics_df)
                )
            )

    delta_df = compute_additional_reduction(final_results_df, target_coverage=0.50)
    expected_delta_rows = (
        expected_network_tasks
        * len(VES_0_LIST)
        * len(R0_LIST)
        * (len(strategies_list) - 1)
        * len(OUTCOME_COLUMNS)
    )
    if STRICT_OUTPUT_VALIDATION and len(delta_df) != expected_delta_rows:
        raise RuntimeError(
            'Incomplete network additional-reduction output: expected {} rows, found {}.'.format(
                expected_delta_rows, len(delta_df)
            )
        )
    ...
    main_plot_df = plot_network_summary(
        summary_df,
        MAIN_TEXT_NETWORK_TYPES,
        str(OUTPUT_DIR / 'Network_Robustness_Qualitative_Summary_MainText.png'),
    )
    all_plot_df = plot_network_summary(
        summary_df,
        ALL_NETWORK_TYPES,
        str(OUTPUT_DIR / 'Network_Robustness_Qualitative_Summary_All.png'),
    )

    raw_name = str(OUTPUT_DIR / 'Network_Robustness_Raw_Results.xlsx')
    ...
    if all_replicates:
        ...
        if STRICT_OUTPUT_VALIDATION:
            ...
            if len(replicate_df) != expected_replicate_rows:
                raise RuntimeError(
                    'Incomplete network replicate output: expected {} rows, found {}.'.format(
                        expected_replicate_rows, len(replicate_df)
                    )
                )
            replicate_keys = [
                'Network_Type', 'Network_Replicate', 'VES_0', 'R0',
                'Coverage', 'Strategy', 'MC_Run'
            ]
            if replicate_df.duplicated(replicate_keys).any():
                raise RuntimeError('Duplicate network replicate records were detected.')
        replicate_name = str(OUTPUT_DIR / 'Network_Robustness_Replicate_Outcomes.csv.gz')
        replicate_df.to_csv(replicate_name, index=False, compression='gzip')
        print('[SAVED] {}'.format(replicate_name), flush=True)

    diag_name = str(OUTPUT_DIR / 'Network_Robustness_Network_Diagnostics.xlsx')
    diagnostics_df.to_excel(diag_name, index=False)
    print('[SAVED] {}'.format(diag_name), flush=True)

    run_configuration_df = pd.DataFrame([{
        'R0_LIST': ','.join(map(str, R0_LIST)),
        'VES_0_LIST': ','.join(map(str, VES_0_LIST)),
        'Coverage levels': ','.join(map(str, COVERAGE_STEPS.tolist())),
        'MC Runs': MC_RUNS_PER_POINT,
        'Simulation Days': SIM_DAYS_STRATEGY,
        'Initial Infectious Nodes': NUM_SEEDS_PER_RUN,
        'Common Random Numbers': USE_COMMON_RANDOM_NUMBERS,
        'Strict Output Validation': STRICT_OUTPUT_VALIDATION,
        'Allow Duplicate Household Member Keys': ALLOW_DUPLICATE_HOUSEHOLD_MEMBER_KEYS,
        'Unique Households': int(df['_household_key'].nunique()),
        'Duplicate Household Member Rows': int(duplicated_member_mask.sum()),
        'Main Network Replicates': NETWORK_REPLICATES_MAIN,
        'SI Network Replicates': NETWORK_REPLICATES_SI,
        'External Prevalence': EXTERNAL_PREVALENCE,
        'IFR_Structure': 'COVID-like baseline age-specific IFR',
        **SCENARIO_PARAMS,
    }])
    ...
    summary_name = str(OUTPUT_DIR / 'Network_Robustness_Additional_Reduction_Summary.xlsx')
    with pd.ExcelWriter(summary_name) as writer:
        delta_df.to_excel(writer, sheet_name='Additional_Reduction', index=False)
        parameter_level_delta_df.to_excel(
            writer, sheet_name='Parameter_Level_Reduction', index=False
        )
    ...
    if SAVE_DYNAMICS_CURVES and all_dynamics:
        ...
        dynamics_name = str(OUTPUT_DIR / 'Network_Robustness_Dynamics_Curves.xlsx')
    ...
    plt.axis('off')
    plt.savefig(str(OUTPUT_DIR / 'Standalone_Legend_Network_Robustness_Strategies.png'), dpi=300, bbox_inches='tight')
    plt.close()
    print('[SAVED] Standalone_Legend_Network_Robustness_Strategies.png', flush=True)

    print('[DONE] Network-structure robustness analysis finished.', flush=True)
